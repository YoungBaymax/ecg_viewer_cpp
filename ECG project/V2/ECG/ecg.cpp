...
void ECG::setbaselinewander()
{
    for(int l=0;l<leads.length();l++)
    {
        QList<int> data = leads.at(l);
        baselinewander *base = new baselinewander();
        QList<int> shift = base->getShift(data);
        QList<int> shiftpoint = base->getShiftpoint(data);
        int num = 0;
        for(int i=0;i<data.length();i++)
        {
            num = num + shift.at(i);
            data.replace(i,data.at(i)-num);
        }
        ...
        //while(true)
        {
            ...
            else
            {
                ...
                {
                    int start = shiftpoint.at(i);
                    int end = shiftpoint.at(i+1);
                    QList<int> range;
                    range.clear();
                    for(int j=0;j<end-start;j++)
                    {
                        range.append(data.at(start+j));
                    }
                    float median = 0;
                    if(range.size()>1)
                    {
        ...
        //qDebug()<<"loop"+QString::number(l);
        data = base->normalizeBaseline(data);
        leads.replace(l,data);
    }
}
...
void ECG::on_pushButton_4_clicked()
{
    //Interpretation
    //Interpret *in = new Interpret(0);
    //in->show();
}
